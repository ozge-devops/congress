...
\section{Discussions}
This paper is a protocol, not a mixer. The public path runs an M3 day-level hop ($\alpha=1$, no portfolio); the analogue hit rate is 49.8\% and the hop does not enter the mixer. Tiers change delivery, not next-day accuracy: T3 and T10 both score 49.7\% on seed 0 because extra tokens are templates. Five-seed accuracy sits next to the majority prior; we do not name a winner. The 2025-2026 bull overlay loses to buy-and-hold; the 2023 holdout does not reverse that claim. Human NASA-TLX is unspecified ($n=0$). Fleiss $\kappa=0.50$ is three codebooks; there are no human annotators. Public text is KAP list teasers, not full KAP HTML.
...
Three items remain open. The 12-participant NASA-TLX study in \texttt{study/} has no responses, so human load and trust are unspecified. There is no human-annotator gold set; polarity is silver codebook output. Full disclosure-detail pages are absent. Those are the next measurements.
